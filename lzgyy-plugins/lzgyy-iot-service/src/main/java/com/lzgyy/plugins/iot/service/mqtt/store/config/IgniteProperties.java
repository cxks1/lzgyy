package com.lzgyy.plugins.iot.service.mqtt.store.config;
...
public class IgniteProperties {
	...
	/**
	 * [apache ignite] 持久化缓存内存初始化大小(MB), 默认值: 64
	 */
	private int persistenceInitialSize = 64;

	/**
	 * [apache ignite] 持久化缓存占用内存最大值(MB), 默认值: 128
	 */
	private int persistenceMaxSize = 128;

	/**
	 * [apache ignite] 持久化磁盘存储路径
	 */
	private String persistenceStorePath;

	/**
	 * [apache ignite] 非持久化缓存内存初始化大小(MB), 默认值: 64
	 */
	private int NotPersistenceInitialSize = 64;

	/**
	 * [apache ignite] 非持久化缓存占用内存最大值(MB), 默认值: 128
	 */
